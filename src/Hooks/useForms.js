...
import { FIREBASE_createEvent, FIREBASE_editEvent } from "../Helpers/FIREBASE/Actions/EventsFirebase";
import { FIREBASE_login, FIREBASE_register, FIREBASE_updateUser } from "../Helpers/FIREBASE/Actions/UserFirebase";
    ...
    const onSubmitFormData = (APPLY = "", ACTION = "", e) => {
        ...
        if (arr_errors.length > 0) {
            ...
            if (APPLY === "AUTH_REGISTER") {
                FIREBASE_register(useFieldsInputsValues.email, useFieldsInputsValues.password);

            } else if (APPLY === "AUTH_LOGIN") {
                FIREBASE_login(useFieldsInputsValues.email, useFieldsInputsValues.password);
            } else if (APPLY === "EVENTS") {
                if (ACTION === "EDIT") {
                    FIREBASE_editEvent(useFieldsInputsValues).then(() => {
                        onCloseModal();
                    });
                } else if (ACTION === "ADD") {
                    FIREBASE_createEvent(useFieldsInputsValues).then(() => {
                        onCloseModal();
                    });
                }
            } else if (APPLY === "FORM_CONFIG") {

                FIREBASE_updateUser(useFieldsInputsValues, onCloseModal);
            }
